...
impl SyncEngine {
    ...
    fn apply_local(
        ...
        if !actions.is_empty() {
            let scan = self.index.scan()?;
            ensure_scan_is_safe(&scan, "pre-apply local")?;
            let latest = MerkleTree::from_records(self.index.sync_records()?)?;
            ensure!(
                latest.root_hash() == current.root_hash() && latest.len() == current.len(),
                "local state changed after its snapshot; retry reconciliation before applying"
            );
        }
...
mod tests {
    ...
    #[test]
    fn local_edit_after_snapshot_blocks_remote_deletion() {
