...
`427320c`의 구현은 독립 리뷰에서 확인된 세 결함을 수정 중이다. 사설 상태와 public
root의 전역 겹침, 거부 전에 디렉터리를 만드는 순서, 이전 증명의 ID 수 제한이
수정 대상이다. 아래 결과는 해당 단계의 실제 명령 로그에서
...
아직 필요한 항목은 v3 권한 리뷰 결함 수정과 재검토, 실제 RW/RO 동기화와
