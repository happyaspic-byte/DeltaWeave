...
there is no configuration-file option or HTTP API.
...
well as receiving v1 `push`. Its help text only mentions pushes. The allow-list
...
Stdout contains pretty-printed JSON; help/version output is plain text. A long-running
...
`Error: ...` on stderr; they are not a stable JSON error envelope.
