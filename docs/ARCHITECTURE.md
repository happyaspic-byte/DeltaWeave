...
The workspace contains eight crates: `deltaweave-core` defines shared types,
...
5. Stage every required content hash in the local CAS before changing either
   ...
   is overwritten.
6. Apply local tombstones deepest-first, remove objects requiring a kind change
...
Local apply uses the initial snapshot plan and does not repeat the receiver's
per-action causal precondition. Remote checks and final rescans detect many
